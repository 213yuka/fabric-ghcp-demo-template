...
| SM 大規模ストレージ形式の有効化 | ❌ なし | Power BI REST API `PATCH https://api.powerbi.com/v1.0/myorg/groups/{groupId}/datasets/{datasetId}` — `{"targetStorageMode": "PremiumFiles"}` |
| SM Approved for Copilot の設定 | ❌ なし（REST API 未提供） | Fabric ポータルで手動設定（SM 設定 → 「Approved for Copilot」チェックボックス） |
...
デモ構築時は **調査系で仕様を確認 → MCP 実行系で構築 → 不足分は REST API で補完** する流れとする。全アイテムの作成・設定を GHCP 内で完結させ、ポータルでの手動設定は最小限（Approved for Copilot のみ）にする。
