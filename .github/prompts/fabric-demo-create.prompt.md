...
> **禁止事項**: ツールの有効/無効を事前にチェックしたり、「ツールが利用できません」「Configure Tools で有効化してください」等のメッセージを表示してはいけない。ターミナル実行やファイル操作が必要な場合は、説明なしにそのまま実行すること。実行時にエラーが出た場合のみ対処する。
...
**ツールの利用可否を確認するステップは不要。** 直接コマンドを実行し、その結果で判断する。
