...
| エージェントインストラクション | トーン、役割、データソース概要、質問カテゴリ別指示、業務用語定義 | REST API: definition の stage_config.json で指定 |
| データソースインストラクション | データソースの目的、必須カラム、結合ロジック、値の形式例、質問タイプ別テーブル・フィルタ指示 | REST API: datasource.json の dataSourceInstructions で指定 |
| 代表質問セット | フィルタリング・結合・集計・日付処理を含む代表的なクエリ例 | REST API: fewshots.json で指定 + 設計書に記載 |

### Data Agent インストラクション設計原則

[公式ベストプラクティス](https://learn.microsoft.com/fabric/data-science/data-agent-configuration-best-practices) §4-10 に基づく:

#### エージェントインストラクション（`stage_config.json` の `aiInstructions`）

以下のセクションを含めて構造化する（§7, §8）:

| セクション | 内容 |
|---|---|
| **Tone and style** | 回答のトーン・スタイル（例: 明確で簡潔なプロフェッショナルな日本語） |
| **General knowledge** | エージェントの役割・責任範囲・フォールバック動作（データ不足時の対応） |
| **Data source descriptions** | 各データソースの概要（何のデータが含まれ、どんな質問に答えられるか） |
| **When asked about** | 質問カテゴリ別のデータソース指示（例: 売上→FactSales、在庫→FactInventory） |
| **業務用語の定義** | 同義語・省略形・ドメイン固有用語の定義（例: 年度=4月〜3月、Q1=4〜6月） |

#### データソースインストラクション（`datasource.json` の `dataSourceInstructions`）
...
| **必須カラム** | 回答に常に含めるべきカラム一覧 |
| **結合ロジック** | テーブル間の結合キーとルール |
| **値の形式例** | 各カラムの代表的な値（略称 vs フルネーム等）。データエージェントはクエリ実行前に行値を参照できないため、フィルタリングのガイドに必須 |
| **When asked about** | 質問タイプ別のテーブル・カラム・フィルタ指示 |
| **SQL/DAX 構文ヒント** | Leading words（§6）。例: `LIKE '%keyword%'` でパターンマッチングをガイド |

#### 代表質問セット（`fewshots.json`）

ベクトル類似性検索でトップ 3 が取得されるため、代表的な質問タイプのクエリ例を含める（§10）:
...
| 原則 | 説明 |
|---|---|
| **「～しない」だけでなく「何をすべきか」を具体的に書く**（§4） | ❌「古いデータを提供しない」→ ✅「最新のレコードを優先して提供する。見つからない場合はその旨を伝える」 |
| **ビジネス用語・省略形・同義語を定義する**（§5） | 暦年 vs 会計年度、Q1=4〜6月、NPS=ネットプロモータースコア等 |
| **SQL/DAX 構文のヒントを含める**（§6） | Leading words で正しいクエリパターンをガイド |
| **不要な詳細を避け、簡潔に書く**（§7） | データソースの歴史や曖昧な指示は含めない |
...
| 同義語 | 曖昧語や同義語に対応できるよう説明・別名を整備する（§5） |
| インストラクション | 構造化されたセクション（Tone/General/When asked about/業務用語）で書く（§7, §8） |
