...
4. Fabric 環境を構築（MCP + Fabric REST API で実行）
   ...
   ↓
5. Fabric ポータルで Data Agent のデータソースを選択（唯一の手動ステップ）
```
...
| **Data Agent** | 自然言語でデータに質問できるエージェント | MCP `onelake_item_create`（データソース選択のみポータル） |
...
MCP + REST API でほぼ全て自動化されるが、Data Agent のデータソース設定のみポータルで行う:

### Data Agent 設定（唯一の手動ステップ）
Data Agent を開き:
