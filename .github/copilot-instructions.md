...
- MCP ツールにない API（Load Table、ワークスペース作成等）は **ターミナルで Fabric REST API を直接呼び出す**（PowerShell 推奨、Python も可）
...
3. 全アイテム（Lakehouse・Semantic Model・Data Agent）の作成・設定が GHCP 内で完結する（ポータルでの手動設定は Approved for Copilot のみ）
