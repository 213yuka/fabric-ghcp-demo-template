...
6. Copilot が自動で:
   ...
   - CSV → Delta 変換、Semantic Model 定義（TMDL）まで自動実行

7. Fabric ポータルで Data Agent のデータソースを選択（唯一の手動ステップ）
...
| Data Agent | 自然言語でデータに質問 | MCP で作成（データソース選択のみポータル） |
...
| 制約 | 詳細 |
|---|---|
| Data Agent のデータソース | 作成後、ポータルで Semantic Model をデータソースとして選択する必要がある（唯一の手動ステップ） |
