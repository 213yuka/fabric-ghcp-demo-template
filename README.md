...
- Semantic Model 作成（スタースキーマ / Direct Lake）
...
3. 両方のサーバーが「✅ 接続済み」になっていることを確認
4. 初回接続時は Microsoft Entra ID の認証が求められる
...
④ MCP ツールで Fabric に直接デプロイ
   ...
   ↓
⑤ Fabric ポータルで初期設定（手動）
   ├── Load to Tables（CSV → Delta テーブル）
   ├── Semantic Model のリレーションシップ設定
...
3. Copilot Chat を **Agent モード** で開く
4. 以下を入力:
...
| Semantic Model | スタースキーマの分析モデル（Direct Lake） | MCP で作成 → ポータルで設定 |
| Data Agent | 自然言語でデータに質問 | MCP で作成 → ポータルで設定 |
...
| 制約 | 詳細 |
|---|---|
| Semantic Model の設定 | MCP でアイテム作成後、テーブル追加・リレーションシップ・メジャーはポータルで手動設定 |
| Data Agent の設定 | MCP でアイテム作成後、データソース・インストラクションはポータルで手動設定 |
| Data Agent の対応範囲 | Semantic Model または Lakehouse に対する自然言語クエリのみ（現時点ではリアルタイムデータ非対応） |
| CSV → Delta 変換 | ポータルの「Load to Tables」で手動実行（Notebook 不要） |
